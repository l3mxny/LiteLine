...
class MapService {
    ...
    private blueLightMarkers: google.maps.Marker[] = [];
     ...
     */
    fitBounds(): void {
        if (this.blueLightMarkers.length === 0 || !this.map) return;
    ...
    showRoute(destLat: number, destLng: number, _destName: string): void {
        ...
        const origin: google.maps.LatLngLiteral = { lat: userPos.lat(), lng: userPos.lng() };
